#!/bin/bash
set -euo pipefail
echo "FILENAME: updateSchemaMDB.sh"
...
if [[ "`wc -l ${TMPFILE}`" < "3" ]]; then
  echo "Dataverse responded with empty solr schema file. When running on K8s: did you bootstrap yet?"
  exit 123
fi

# WST: ${TARGET} points to solr schema.xml file which will be updated by the update-fields.sh script.
echo "Calling update-fields.sh from updateSchemaMDB.sh: update-fields.sh -p {COLLECTION_DIR}/conf/schema.xml ${TMPFILE}"
${SCHEMA_SCRIPT_DIR}/update-fields.sh -p ${TARGET}/schema.xml ${TMPFILE}
